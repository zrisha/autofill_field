...
/**
 * Base class for unit tests for profilefield_autofill.
 *
...
/**
 * Unit tests for user\profile\field\autofill\classes\privacy\provider.php
 *
 * @copyright  2018 Mihail Geshoski <mihail@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class profilefield_autofill_testcase extends provider_testcase {
    ...
    public function test_get_contexts_for_userid() {
        ...
        $profilefieldid = $this->add_profile_field($categoryid, 'autofill');
        // Create a user.
        ...
        $contextlist = \profilefield_autofill\privacy\provider::get_contexts_for_userid($user->id);
    ...
    public function test_export_user_data() {
        // Create profile category.
        $categoryid = $this->add_profile_category();
        // Create autofill profile field.
        $autofillprofilefieldid = $this->add_profile_field($categoryid, 'autofill');
        // Create checkbox profile field.
        $checkboxprofilefieldid = $this->add_profile_field($categoryid, 'checkbox');
        // Create a user.
        $user = $this->getDataGenerator()->create_user();
        $context = context_user::instance($user->id);
        // Add autofill user info data.
        $this->add_user_info_data($user->id, $autofillprofilefieldid, 'test autofill');
        // Add checkbox user info data.
        $this->add_user_info_data($user->id, $checkboxprofilefieldid, 'test data');
        $writer = \core_privacy\local\request\writer::with_context($context);
        $this->assertFalse($writer->has_any_data());
        $this->export_context_data_for_user($user->id, $context, 'profilefield_autofill');
        $data = $writer->get_data([get_string('pluginname', 'profilefield_autofill')]);
        $this->assertCount(3, (array) $data);
        $this->assertEquals('Test field', $data->name);
        $this->assertEquals('This is a test.', $data->description);
        $this->assertEquals('test autofill', $data->data);
    }
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        \profilefield_autofill\privacy\provider::delete_data_for_all_users_in_context($context);
        // Check that the correct profile field has been deleted.
        $userinfodata = $DB->get_records('user_info_data', ['userid' => $user->id]);
        $this->assertCount(1, $userinfodata);
        $this->assertNotEquals('test autofill', reset($userinfodata)->data);
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $userinfodata = $DB->get_records('user_info_data', ['userid' => $user->id]);
        $this->assertCount(2, $userinfodata);
        $approvedlist = new \core_privacy\local\request\approved_contextlist($user, 'profilefield_autofill',
            [$context->id]);
        \profilefield_autofill\privacy\provider::delete_data_for_user($approvedlist);
        // Check that the correct profile field has been deleted.
        $userinfodata = $DB->get_records('user_info_data', ['userid' => $user->id]);
        $this->assertCount(1, $userinfodata);
        $this->assertNotEquals('test autofill', reset($userinfodata)->data);
